	...
	public sealed class WaveIn : IDisposable
	{
		...
		public void Open(WaveFormat waveFormat)
		{
			lock (this.startStopLock)
			{
				...
				NativeMethods.Throw(NativeMethods.waveInOpen(ref intPtr, (uint)this.deviceId, ref waveformatex, this.callback, (IntPtr)0, NativeMethods.WAVEOPENFLAGS.CALLBACK_WINDOW | NativeMethods.WAVEOPENFLAGS.CALLBACK_THREAD | NativeMethods.WAVEOPENFLAGS.WAVE_FORMAT_DIRECT), NativeMethods.ErrorSource.WaveOut);
				this.handle = new WaveInSafeHandle(intPtr);
			}
		}
		...
		private void AddBuffer()
		{
			...
			NativeMethods.Throw(NativeMethods.waveInPrepareHeader(this.handle, intPtr2, (uint)Marshal.SizeOf(typeof(NativeMethods.WAVEHDR))), NativeMethods.ErrorSource.WaveOut);
			NativeMethods.Throw(NativeMethods.waveInAddBuffer(this.handle, intPtr2, (uint)Marshal.SizeOf(typeof(NativeMethods.WAVEHDR))), NativeMethods.ErrorSource.WaveOut);
			lock (this.bufferingLock)
			{
